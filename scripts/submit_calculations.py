...
if __name__ == "__main__":
    ...
    # Check if a specific batch ID is provided as an argument
    specific_batch_id = sys.argv[2] if len(sys.argv) > 2 else None

    conn = connect_db()
    cursor = conn.cursor()

    if specific_batch_id:
        cursor.execute("SELECT DISTINCT BatchID FROM MoleculeData WHERE BatchID = ?", (specific_batch_id,))
        result = cursor.fetchone()
        if not result:
            print(f"Batch ID {specific_batch_id} does not exist.")
            sys.exit(1)
        batch_files = [specific_batch_id]
    else:
        # If no specific batch ID is provided, select batches based on the calculation stage
        stage_mapping = {
        'storage': 'storage',
        'tbr': 'storage_completed',
        'abs': 'storage_completed, ts_completed',
        'storage_dft': 'storage_completed, ts_completed, abs_completed',
        'tbr_dft': 'storage_completed', 
        'abs_dft': 'storage_completed, ts_completed',
        }
